...
class ScorePlot:
    ...
    def load_propaty(self):
        ...
        self.score = self.json_dict["mobidb_consensus"]["disorder"]["predictors"][1][
            "scores"
        ]  # scoreの値を取得する
        ...
        self.text = (
            ...
            + "\n"
            + "Percentage (High):"
            + str(round(div / len(self.score) * 100, 3))
            + "%"
        )
    ...
    def set_plt(self):
        ...
        for i in range(len(self.score)):
            ax.annotate(
                self.sequence[i], (i, 1.05), size=8, horizontalalignment="center"
            )
